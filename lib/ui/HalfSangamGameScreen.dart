...
class HalfSangamGameScreen extends StatefulWidget {
  final GameData gameData;
  final String gameType;
  final int gameConstant;
  final String bettingOpenTime;
  final String bettingCloseTime;

  HalfSangamGameScreen(
      {@required this.gameData,
      @required this.gameType,
      @required this.gameConstant,
      @required this.bettingOpenTime,
      @required this.bettingCloseTime});

  @override
  _HalfSangamGameScreenState createState() => _HalfSangamGameScreenState();
}

class _HalfSangamGameScreenState extends State<HalfSangamGameScreen> {
  GlobalKey<AutoCompleteTextFieldState<String>> key = new GlobalKey();

  GameData gameData;
  String gameType;
  int gameConstant;
  String bettingOpenTime;
  String bettingCloseTime;
  String authToken = "";
  ...
  TextEditingController pannaController;
  ...
  void initState() {
    ...
    bettingOpenTime = widget.bettingOpenTime;
    bettingCloseTime = widget.bettingCloseTime;
    ...
    pannaController = TextEditingController();
                    ...
                    Row(
                      children: [
                        ...
                        InkWell(
                          ...
                          onTap: () {
                            setState(() {
                              _isOpenSelected = !_isOpenSelected;
                            });
                          },
                        )
                      ],
                    ...
                    Row(
                      ...
                      children: [
                        Container(
                          ...
                          alignment: Alignment.center,
                          child: Text(
                            _isOpenSelected ? 'OPEN DIGIT' : 'CLOSE DIGIT',
                            style: TextStyle(
                                color: Colors.white,
                                fontSize: 18,
                                fontWeight: FontWeight.bold),
                    ...
                    Row(
                      ...
                      children: [
                        Container(
                          ...
                          alignment: Alignment.center,
                          child: Text(
                            _isOpenSelected ? 'CLOSE PANNA' : 'OPEN PANNA',
                            style: TextStyle(
                                color: Colors.white,
                                fontSize: 18,
                                fontWeight: FontWeight.bold),
                      ...
                      child: MaterialButton(
                        ...
                        onPressed: () {
                          if (digitController.text.isNotEmpty &&
                              pannaController.text.isNotEmpty &&
                            ...
                            if (digitController.text.isEmpty)
                              showErrorDialog('Please enter digit to play');
                            else if (pannaController.text.isEmpty)
                              showErrorDialog('Please select panna to play');
                            else if (pointsController.text.isEmpty)
                              showErrorDialog('Please enter point to play');
  ...
  void addToPlayedGames() {
    if (_isOpenSelected)
      selectedGameTime = bettingOpenTime;
    else
      selectedGameTime = bettingCloseTime;

    if (int.parse(pointsController.text.trim()) < int.parse(walletBalance) && int.parse(pointsController.text.trim()) >= int.parse(minBetAmount)) {
      if (digitController.text.isNotEmpty &&
          pointsController.text.isNotEmpty &&
          checkPannaValueExists(pannaController.text.trim()) &&
          selectedDate.isNotEmpty &&
          selectedGameTime.isNotEmpty) {
        playedDigit = int.parse(digitController.text.trim());
        pointsPlayed = int.parse(pointsController.text.trim());
        pannaPlayed = int.parse(pannaController.text.trim());

        playedDigit = int.parse('$pannaPlayed$playedDigit');
        ...
        PlayedGameModel playedGameModel = PlayedGameModel(
            ...
            gameTime: selectedGameTime, finalAmount: walletBalance);
        ...
        if (selectedDate.isEmpty)
          ...
          showErrorDialog('Please enter panna to play on');
        else if (!checkPannaValueExists(pannaController.text))
          showErrorDialog('Panna not available');
      }
    } else {
